%macro ds_merge() / parmbuff store source;
/*-----------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------*/
/* MACRO DESCRIPTION:                                                          */
/*    Acts as a regular DATA step MERGE statements excepts creates ATTRIB      */
/*    statements for all variables so that shared variables have a length of   */
/*    the maximum between the two to prevent truncation.                       */
/*                                                                             */
/* Notable similarities with MERGE statement:                                  */
...
/*-----------------------------------------------------------------------------*/
/* USAGE DOCUMENTATION:                                                        */
/*  - Use %DS_MERGE in the DATA step as you would the MERGE statement.         */
/*                                                                             */
/*  - Supply a comma separated list of each of the datasets that you would     */
/*    put on a MERGE statement                                                 */
/*                                                                             */
/*  - If applicable, supply DROP=, KEEP=, RENAME= options as you would in      */
/*    any other step (in parentheses after the data set)                       */
/*                                                                             */
/*  - Example given below (extra carriage returns to fit in the comment block) */
/*                                                                             */
/*    data CONCAT_TABLES;                                                      */
/*       %DS_MERGE(npmext.table1(drop=var1 rename=(varA=var2))                 */
/*                ,npmext.table2(keep=var1-var3 (rename=(var2=NEWVAR2)))       */
/*                 );                                                          */
/*    run;                                                                     */
/*                                                                             */
...
/*    BEFORE the %DS_MERGE, then (similar to when using a MERGE statement),    */
/*    that first assignment initializes the variable, not %DS_MERGE            */
/*                                                                             */
/*-----------------------------------------------------------------------------*/
